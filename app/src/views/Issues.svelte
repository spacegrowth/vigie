<script lang="ts">
  // Issues: issue_opened | issue_commented, in the Comments card style
  // (docs/CONTRACT.md's Navigation section: "Issues... in the Comments
  // card style" — no dedicated Issues.dc.html mockup exists, so this
  // reuses Comments.dc.html's row anatomy; noted as a deliberate deviation
  // in the side-by-side report).
  ...
  import { ISSUE_KINDS, commentRefLabel, eventDetailTarget } from "../lib/threads";
  ...
  async function load() {
    ...
    try {
      ...
      ).filter((e) => ISSUE_KINDS.includes(e.kind));
    } finally {
      loading = false;
    }
  }
...
<div class="head">
  ...
  <span class="muted">opened and commented</span>
</div>
...
<div class="body" use:scroller>
  ...
  {:else if events.length === 0}
    <div class="zero-state">
      <p class="muted">
        {scopeEmptyMessage("issue activity", { repoLabel: filterRepoLabel, teamLabel: filterTeamLabel, mode: effectiveMode }) ??
        ...
        <div class="card-wrap">
          <button class="card" class:unseen={!e.seen} onclick={() => open(e)}>
            <div class="row">
              ...
              {#if e.kind === "issue_opened"}
                <span class="ref title-text">{e.title}</span>
              {:else}
                <span class="muted ref">{commentRefLabel(e)}</span>
              {/if}
              <span class="muted number">#{e.number}</span>
              <span class="spacer"></span>
